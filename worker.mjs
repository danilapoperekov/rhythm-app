...
function model(env) {
  return env.RHYTHM_AI_MODEL || 'gpt-5.6-terra';
}

async function openaiJson(env, body) {
  ...
  return { ok: true, text: data.output_text || '', data };
}

async function reflect(request, env) {
  if (!env.OPENAI_API_KEY) return send(request, env, 503, { error: 'AI_NOT_CONFIGURED', message: 'ИИ-сервер пока не настроен.' });
  let input;
  try {
  ...
  const upstream = await openaiJson(env, {
    model: model(env),
  ...
  return send(request, env, 200, { result: upstream.text, model: model(env) });
}
...
async function meditation(request, env) {
  ...
  if (!upstream.ok) return send(request, env, upstream.status, { error: 'AI_UPSTREAM_ERROR', message: upstream.error });
  return send(request, env, 200, JSON.parse(upstream.text || '{}'));
}
...
async function capture(request, env) {
  if (!env.OPENAI_API_KEY) return send(request, env, 503, { error: 'AI_NOT_CONFIGURED', message: 'ИИ-сервер пока не настроен. Голос и черновик остались на устройстве.' });
  ...
  return send(request, env, 200, { transcript, proposals: Array.isArray(parsed.proposals) ? parsed.proposals : [], model: model(env) });
}
...
async function route(request, env) {
  ...
  if (request.method === 'GET' && path === '/api/health') return send(request, env, 200, { ok: true });
