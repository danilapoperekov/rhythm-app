...
(() => {
  ...
  function renderToday() {
    ...
    const todayTasks = state.tasks.filter((task) => task.date === today && !task.cancelled).sort((a, b) => a.time.localeCompare(b.time));
  ...
  function renderCalendar() {
    ...
    const todayTasks = state.tasks.filter((task) => task.date === today && !task.cancelled).sort((a, b) => a.time.localeCompare(b.time));
    const upcomingTasks = state.tasks.filter((task) => task.date > today && !task.done && !task.cancelled).sort((a, b) => a.date.localeCompare(b.date) || a.time.localeCompare(b.time));
    ...
    const taskRows = source.length ? source.map((task) => `<article class="plan-task ${task.done ? 'done' : ''} ${task.cancelled ? 'cancelled' : ''} ${task.id === selectedTaskId ? 'selected' : ''}" data-task-select="${task.id}"><button class="task-toggle ${task.done ? 'done' : ''}" data-task-toggle="${task.id}" aria-label="${task.cancelled ? 'Вернуть отменённую задачу' : task.done ? 'Вернуть задачу' : 'Завершить задачу'}">${task.done ? '✓' : task.cancelled ? '↺' : ''}</button><div class="plan-task-main"><div class="plan-task-title">${esc(task.title)}</div><div class="plan-task-meta"><span class="plan-category" style="background:${categoryColor(task.category)}"></span>${task.cancelled ? 'Отменена · ' : ''}${task.date === today ? 'Сегодня' : formatDate(task.date)}${task.time ? ` · ${task.time}` : ''} · ${categoryName(task.category)}</div></div><button class="plan-row-more" data-task-select="${task.id}" aria-label="Открыть задачу">›</button></article>`).join('') : '<div class="plan-empty"><div>✓</div><b>Здесь свободно</b><span>Добавьте задачу или оставьте место для себя.</span></div>';
    const detail = selectedTask ? `<section class="plan-detail-card ${selectedTask.cancelled ? 'cancelled' : ''}"><div class="plan-detail-top"><span class="plan-detail-category" style="background:${categoryColor(selectedTask.category)}">${selectedTask.cancelled ? 'Отменена' : categoryName(selectedTask.category)}</span><button class="icon-btn" data-task-toggle="${selectedTask.id}" aria-label="${selectedTask.cancelled ? 'Вернуть отменённую задачу' : selectedTask.done ? 'Вернуть задачу' : 'Завершить задачу'}">${selectedTask.done ? '✓' : selectedTask.cancelled ? '↺' : '○'}</button></div><h2 class="plan-detail-title ${selectedTask.done || selectedTask.cancelled ? 'done' : ''}">${esc(selectedTask.title)}</h2><div class="detail-line"><span>◷</span><div><b>${selectedTask.date === today ? 'Сегодня' : formatDate(selectedTask.date, true)}</b><small>${selectedTask.time || 'Время не выбрано'}</small></div></div><div class="detail-line"><span>◌</span><div><b>${categoryName(selectedTask.category)}</b><small>Контекст задачи</small></div></div><div class="plan-detail-note"><span>Заметка</span><p>${esc(selectedTask.note || 'Добавьте детали, когда они помогут начать. Сейчас достаточно следующего шага.')}</p></div><div class="form-actions plan-task-actions"><button class="btn btn-secondary" data-task-edit="${selectedTask.id}">Редактировать</button><button class="btn btn-ghost" data-task-move="today" data-task-id="${selectedTask.id}" ${selectedTask.date === today ? 'disabled' : ''}>На сегодня</button><button class="btn btn-ghost" data-task-move="tomorrow" data-task-id="${selectedTask.id}">На завтра</button><button class="btn ${selectedTask.cancelled ? 'btn-secondary' : 'btn-danger'}" data-task-cancel="${selectedTask.id}">${selectedTask.cancelled ? 'Вернуть задачу' : 'Отменить'}</button></div></section>` : `<section class="plan-detail-empty"><div>◌</div><h2>Выберите задачу</h2><p>Здесь появится её контекст, чтобы держать в фокусе только одно важное дело.</p></section>`;
    ...
    app.innerHTML = `<div class="page">
      ...
      <section class="planner-shell">
        <aside class="planner-lists"><div class="planner-lists-head">Мои списки</div><button class="planner-list ${planMode === 'today' && planFilter === 'today' ? 'active' : ''}" data-plan-filter="today"><span class="planner-list-icon blue">☼</span><span>Сегодня</span><b>${todayTasks.filter((task) => !task.done).length}</b></button><button class="planner-list ${planMode === 'today' && planFilter === 'upcoming' ? 'active' : ''}" data-plan-filter="upcoming"><span class="planner-list-icon purple">◷</span><span>Предстоит</span><b>${upcomingTasks.length}</b></button><button class="planner-list ${planMode === 'today' && planFilter === 'all' ? 'active' : ''}" data-plan-filter="all"><span class="planner-list-icon gray">≡</span><span>Все задачи</span><b>${allTasks.filter((task) => !task.done && !task.cancelled).length}</b></button><div class="planner-divider"></div><div class="planner-lists-head">Пространства</div><button class="planner-list ${planMode === 'today' && planFilter === 'self' ? 'active' : ''}" data-plan-filter="self"><span class="planner-list-icon orange">⌂</span><span>Личное</span><b>${state.tasks.filter((task) => task.category === 'self' && !task.done && !task.cancelled).length}</b></button><button class="planner-list ${planMode === 'today' && planFilter === 'health' ? 'active' : ''}" data-plan-filter="health"><span class="planner-list-icon green">✦</span><span>Здоровье</span><b>${state.tasks.filter((task) => task.category === 'health' && !task.done && !task.cancelled).length}</b></button><button class="planner-list ${planMode === 'today' && planFilter === 'work' ? 'active' : ''}" data-plan-filter="work"><span class="planner-list-icon navy">□</span><span>Работа</span><b>${state.tasks.filter((task) => task.category === 'work' && !task.done && !task.cancelled).length}</b></button></aside>
  ...
  const taskForm = {
    ...
    body: `<form id="task-form" class="form-grid"><div class="field full"><label for="task-title">Что нужно сделать</label><input id="task-title" name="title" placeholder="Название задачи" required maxlength="100"></div><div class="field"><label for="task-date">Дата</label><input id="task-date" name="date" type="date" value="${selectedDate}" required></div><div class="field"><label for="task-time">Время</label><input id="task-time" name="time" type="time" value="09:00"></div><div class="field full"><label for="task-category">Область</label><select id="task-category" name="category"><option value="self">Личное</option><option value="health">Здоровье</option><option value="work">Работа</option></select></div><div class="field full"><label for="task-note">Заметка</label><textarea id="task-note" name="note" placeholder="Следующий шаг, контекст или что важно не забыть"></textarea></div>${formActions('Добавить задачу')}</form>`
  };

  function taskEditForm() {
    const task = state.tasks.find((item) => item.id === selectedTaskId);
    if (!task) return { head: modalHead('Задача не найдена', 'Вернитесь к списку задач и выберите нужную.'), body: '<div class="form-actions"><button class="btn btn-primary" type="button" data-close-modal>Закрыть</button></div>' };
    return {
      head: modalHead('Редактировать задачу', 'Измените детали или перенесите задачу. История останется на этом устройстве.'),
      body: `<form id="task-edit-form" class="form-grid"><input type="hidden" name="id" value="${task.id}"><div class="field full"><label for="task-edit-title">Что нужно сделать</label><input id="task-edit-title" name="title" value="${esc(task.title)}" required maxlength="100"></div><div class="field"><label for="task-edit-date">Дата</label><input id="task-edit-date" name="date" type="date" value="${task.date}" required></div><div class="field"><label for="task-edit-time">Время</label><input id="task-edit-time" name="time" type="time" value="${task.time || ''}"></div><div class="field full"><label for="task-edit-category">Область</label><select id="task-edit-category" name="category"><option value="self" ${task.category === 'self' ? 'selected' : ''}>Личное</option><option value="health" ${task.category === 'health' ? 'selected' : ''}>Здоровье</option><option value="work" ${task.category === 'work' ? 'selected' : ''}>Работа</option></select></div><div class="field full"><label for="task-edit-note">Заметка</label><textarea id="task-edit-note" name="note" placeholder="Следующий шаг, контекст или что важно не забыть">${esc(task.note || '')}</textarea></div>${formActions('Сохранить изменения')}</form>`
    };
  }
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (taskToggle) {
      ...
      if (task) {
        if (task.cancelled) { task.cancelled = false; task.done = false; saveState('Задача возвращена в план'); }
        else { task.done = !task.done; saveState(task.done ? 'Задача выполнена' : 'Задача возвращена'); }
        renderers[currentView]();
      } return;
    }
    const taskEdit = event.target.closest('[data-task-edit]');
    if (taskEdit) { selectedTaskId = taskEdit.dataset.taskEdit; openModal('taskEdit'); return; }
    const taskCancel = event.target.closest('[data-task-cancel]');
    if (taskCancel) {
      const task = state.tasks.find((item) => item.id === taskCancel.dataset.taskCancel);
      if (task) { task.cancelled = !task.cancelled; if (task.cancelled) task.done = false; saveState(task.cancelled ? 'Задача отменена — её можно вернуть в «Все задачи»' : 'Задача возвращена в план'); renderCalendar(); }
      return;
    }
    const taskMove = event.target.closest('[data-task-move]');
    if (taskMove) {
      const task = state.tasks.find((item) => item.id === taskMove.dataset.taskId);
      if (task) { task.date = taskMove.dataset.taskMove === 'tomorrow' ? dateKey(addDays(new Date(), 1)) : dateKey(new Date()); task.cancelled = false; selectedDate = task.date; saveState('Задача перенесена'); renderCalendar(); }
      return;
    }
  ...
  document.addEventListener('submit', async (event) => {
    ...
    if (event.target.id === 'api-connection-form') {
      ...
      state.tasks.push({ id: uid('task'), title: data.title.trim(), date: data.date, time: data.time, category: data.category, note: data.note.trim(), done: false, cancelled: false });
      selectedDate = data.date; saveState('Задача добавлена'); closeModal(); renderers[currentView]();
    } else if (event.target.id === 'task-edit-form') {
      const task = state.tasks.find((item) => item.id === data.id);
      if (!task) { toast('Задача не найдена'); return; }
      task.title = data.title.trim(); task.date = data.date; task.time = data.time; task.category = data.category; task.note = data.note.trim();
      selectedDate = task.date; saveState('Изменения задачи сохранены'); closeModal(); renderCalendar();
